...
def inf(checkpoint_file, file=None, numpy=None, tensor=None, device=None):
    # Check arguments
    if file is None and numpy is None and tensor is None:
...
def compare():
    with torch.no_grad():
        # Prepare save path
        base_path = '../Data/Compare'
        if not os.path.exists(base_path):
            raise FileNotFoundError('./Compare folder does not exist.')
        if not os.path.exists(os.path.join(base_path, 'Images')):
            os.mkdir(os.path.join(base_path, 'Images'))
        
        plot_names = ['z=83', 'z=99', 'z=123', 'Sagittal', 'Coronal', 'Axial']
        colors = ['red', 'green', 'blue', 'orange', 'cyan', 'violet', 'brown', 'black']
        for sim_number in [2, 1]:
            checkpoint_files = [os.path.join(base_path, f) for f in os.listdir(base_path) if os.path.isfile(os.path.join(base_path, f)) and f.split('.')[-1]=='mat']
            ...
            ax.set_title(f'Sim{sim_number} | Overall Brain')
            ax.set_ylabel('Susceptibility Statistics')
            
            # Checkpoint inferences
            for num, checkpoint in enumerate(checkpoint_files):
                recon = loadmat(checkpoint)[f'chi{sim_number}']
                recon_mean = np.mean(recon)
                recon_std = np.std(recon)
                ax.errorbar(num+1, recon_mean, recon_std, capsize=10, markersize=7, marker='o', color=colors[num], ecolor=colors[num], mfc=colors[num], mec=colors[num], label=checkpoint.split('/')[-1][:-4])

            # MEDI reconstruction
            with h5py.File(f'../Data/Conventional/Sim{sim_number}_MEDI_fix.mat') as f:
                medi = f['Sus_medi'].__array__()
                medi_mean = np.mean(medi)
                medi_std = np.std(medi)
                num = len(checkpoint_files)
                ax.errorbar(num+1, medi_mean, medi_std, capsize=10, markersize=7, marker='o', color=colors[num], ecolor=colors[num], mfc=colors[num], mec=colors[num], label='MEDI')
            
            plt.legend(bbox_to_anchor=(0., -0.12-0.1*(len(checkpoint_files)//4), 1., -0.102-0.1*(len(checkpoint_files)//4)), loc=8, ncol=4, mode="expand", borderaxespad=0., fancybox=True, shadow=True)
            plt.tight_layout()
            plt.savefig(os.path.join(base_path, 'Images', f'Sim{sim_number}_overall.png'))

            # Visualize important slices
            # Helper object and function
            nonorm = matplotlib.colors.NoNorm()
            def ax_show_slice(num_row, axis, slice):
                if num_row == 0:
                    axis.imshow(np.fliplr(np.rot90(recon[:,:,83], -1)), cmap='gray', norm=nonorm)
                elif num_row == 1:
                    axis.imshow(np.fliplr(np.rot90(recon[:,:,99], -1)), cmap='gray', norm=nonorm)
                elif num_row == 2:
                    axis.imshow(np.fliplr(np.rot90(recon[:,:,123], -1)), cmap='gray', norm=nonorm)
                elif num_row == 3:
                    axis.imshow(np.fliplr(np.rot90(recon[73+num_img,:,:])), cmap='gray', norm=nonorm)
                elif num_row == 4:
                    axis.imshow(np.rot90(recon[:,110-num_img,:]), cmap='gray', norm=nonorm)
                elif num_row == 5:
                    axis.imshow(np.fliplr(np.rot90(recon[:,:,134+num_img], -1)), cmap='gray', norm=nonorm)

            # Show 15 slices each for Sim1 and Sim2
            for num_img in range(15):
                plt.clf()
                plt.close('all')
                plt.rcParams.update({'font.size': 20.0})
                fig, ax = plt.subplots(6, len(checkpoint_files)+1, figsize=(len(checkpoint_files)*4, 20))
                for num_row, ax_row in enumerate(ax):
                    for axis in ax_row:
                        axis.xaxis.set_visible(False)
                        axis.set_yticks([])
                        axis.set_yticklabels([])
                    ax[num_row][0].set_ylabel(plot_names[num_row])
                plt.suptitle(f'Sim{sim_number} | Slices')

                for num_col, checkpoint in enumerate(checkpoint_files):
                    recon = loadmat(checkpoint)[f'chi{sim_number}']
                    recon = np.clip(recon, -0.1, 0.1) * 5 + 0.5
                    ax[0][num_col].set_title(checkpoint.split('/')[-1][:-4])
                    for num_row in range(6):
                        ax_show_slice(num_row, ax[num_row][num_col], recon)

                with h5py.File(f'../Data/Conventional/Sim{sim_number}_MEDI_fix.mat') as f:
                    medi = f['Sus_medi'].__array__()
                    medi = np.clip(medi, -0.1, 0.1) * 5 + 0.5
                    ax[0][-1].set_title('MEDI')
                    for num_row in range(6):
                        ax_show_slice(num_row, ax[num_row][-1], medi)

                plt.tight_layout(rect=(0, 0, 1, 0.97))
                plt.savefig(os.path.join(base_path, 'Images', f'Sim{sim_number}_slices_{num_img+1:02d}.png'))
            
            # Save GIF
            images = []
            for i in range(15):
                images.append(imageio.imread(os.path.join(base_path, 'Images', f'Sim{sim_number}_slices_{i+1:02d}.png')))
            imageio.mimsave(os.path.join(base_path, 'Images', f'Sim{sim_number}_slices.gif'), images, duration=0.5)
...
def inference(model_architecture, experience_name, model_name):
    with torch.no_grad():
        ...
        for batch, (b, y, _) in enumerate(test_loader):
            ...
            # Save MATLAB array
            save_mat_dict[f'y{batch//4+9}{"".join(filter(str.isalnum, image_names[batch%4]))}'] = y.numpy().squeeze()
            save_mat_dict[f'chi{batch//4+9}{"".join(filter(str.isalnum, image_names[batch%4]))}'] = chi.numpy().squeeze()

            # Save NIFTI image
            save_nii(y.squeeze(), evaluation_save_path, f'Test{batch//4+9:02d}_label_{image_names[batch%4]}')
            save_nii(chi.squeeze(), evaluation_save_path, f'Test{batch//4+9:02d}_inference_{image_names[batch%4]}')
...
if __name__=='__main__':
    if len(sys.argv) != 2:
        ...
        print(f'Checkpoint verified! Begin inference on {device} with {inference_data_path}.')

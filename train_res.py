...
def evaluate_model(name, network, loader, step, save_path, writer=None):
    with torch.no_grad():
        ...
        chi_res_mean = loader.__dict__['dataset'].RES_mean
        chi_res_std = loader.__dict__['dataset'].RES_std

        network.eval()
        for batch, (b, y, m, chi_conven) in enumerate(loader):
            b, y, m, chi_conven = b.to(device), y.to(device), m.to(device), chi_conven.to(device)
            chi_res = network(b)

            b = b * b_std + b_mean
            y = y * y_std + y_mean
            chi_res = chi_res * chi_res_std + chi_res_mean
            
            chi_conven = chi_conven.permute(0, 4, 1, 2, 3)

            chi = chi_conven + chi_res
...
def train(load_path=None):
    ...
    train_dataset = QSMDataset('res_train')
    ...
    val_loader = DataLoader(QSMDataset('res_val'), batch_size=1, shuffle=False)
    ...
    for epoch in range(start_epoch, start_epoch + train_epochs):
        ...
        for batch, (b, y, m) in enumerate(train_loader):
            ...
            loss, loss_parts = total_loss(chi_res, y, b, d, m, b_mean, b_std, y_mean, y_std)
            loss.backward()
            step += 1
            optimizer.step()

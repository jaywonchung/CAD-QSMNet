...
%---------------------------customize section-----------
DATA_NAME='../../Data/Train_0826_maskAug.mat';
training_before_Address='../../Data/training_set_before_patch.mat';
maskAug_before_Address='../../Data/maskAug_set_before_patch.mat';
...
for patientNumber=patientSet
    ...
    eval(sprintf('current_phs=multiphs%d;',patientNumber));
    eval(sprintf('current_aug=multicosaug%d;',patientNumber));
    for i=1:5
        current_cosmos(:,:,:,i)=current_cosmos(:,:,:,i);
        current_phase(:,:,:,i)=current_phs(:,:,:,i);
        current_cosmos(:,:,:,i+5)=current_aug(:,:,:,i);
        current_phase(:,:,:,i+5)=ifftn(fftn(current_aug(:,:,:,i)).*D);
    end

...
for patientNumber=patientSet
    ...
    mask_1=Mask;
    for i=1:10
        maskAug_stat=mod(i-1,5)+1;
        mask_2(:,:,:,i)=mask_1(:,:,:,maskAug_stat);
    end
    ...
    data_cosmos=data_cosmos(:,:,:,1:10);
    data_phase=data_phase(:,:,:,1:10);
    
    zeroMask=zeros(size(mask_2));
    mask_2=zeroMask+mask_2;
    mask_2(mask_2==0)=NaN;
    data_cosmos=data_cosmos.*mask_2;
    data_phase=data_phase.*mask_2;

...
%---------------------------customize section-----------
DATA_NAME='../../Data/Train_0704_17640_2_4.mat';
...
for patientNumber=patientSet
    ...
    for i=1:5
        current_cosmos(:,:,:,i+5)=lambda_1*current_cosmos(:,:,:,i);
        current_phase(:,:,:,i+5)=ifftn(fftn(current_cosmos(:,:,:,i+5)).*D);
    end
    for i=1:5
        current_cosmos(:,:,:,i+10)=lambda_2*current_cosmos(:,:,:,i);
        current_phase(:,:,:,i+10)=ifftn(fftn(current_cosmos(:,:,:,i+10)).*D);
...
%just making pseudo iterator to facilitate patching process
PP = 3; PP_y = 7; SS = 6; PS = 64; %how many slices along x,y,z and patch size 64 
str_x = (174-PS)/(PP-1); %stride along x
str_y = (172-PS)/(PP_y-1); %stride along y
str_z = (159-PS)/(SS-1); %stride along z
[yy, xx] = meshgrid(1:PP_y,1:PP); xx = repmat(xx,[1,1,SS]); yy = repmat(yy,[1,1,SS]);
...
for patientNumber=patientSet
    ...
    for aug = 6:15
        data_L = data1(2:175,3:174,1:159,(aug));
        for jj=1:tt
             maskPatches(:,:,:,patientNumber,ori+tt*(aug-6)+jj) = data_L(str_x*xx(jj)-(str_x-1):str_x*xx(jj)+(PS-str_x),...
                str_y*yy(jj)-(str_y-1):str_y*yy(jj)+(PS-str_y),str_z*zz(jj)-(str_z-1):str_z*zz(jj)+(PS-str_z));
        end
    end
end
...
for patientNumber=patientSet
    ...
    for aug = 6:15 
        data_I = data2(2:175,3:174,1:159,(aug));%data_l is edge trimed phase data
        for jj=1:tt
            data_input(:,:,:,patientNumber,ori+tt*(aug-6)+jj) = data_I(str_x*xx(jj)-(str_x-1):str_x*xx(jj)+(PS-str_x),...
                str_y*yy(jj)-(str_y-1):str_y*yy(jj)+(PS-str_y),str_z*zz(jj)-(str_z-1):str_z*zz(jj)+(PS-str_z));
        end 
    end
end
...
for patientNumber=patientSet
    ...
    for aug = 6:15
        data_L = data1(2:175,3:174,1:159,(aug));%data_L is edge trimmed susceptibility
        for jj=1:tt
            data_label(:,:,:,patientNumber,ori+tt*(aug-6)+jj) = data_L(str_x*xx(jj)-(str_x-1):str_x*xx(jj)+(PS-str_x),...
                str_y*yy(jj)-(str_y-1):str_y*yy(jj)+(PS-str_y),str_z*zz(jj)-(str_z-1):str_z*zz(jj)+(PS-str_z));
        end 
    end
end
